...
import (
	...
	"github.com/stockyard-dev/stockyard-codex/internal/license"
	"github.com/stockyard-dev/stockyard-codex/internal/server"
	"github.com/stockyard-dev/stockyard-codex/internal/store"
)

func main() {
	port := getEnv("PORT", "9120")
	dataDir := getEnv("DATA_DIR", "./data")
	licenseKey := os.Getenv("CODEX_LICENSE_KEY")
	...
	log.Printf("Stockyard Codex listening on :%s (tier: %s)", port, tier)
